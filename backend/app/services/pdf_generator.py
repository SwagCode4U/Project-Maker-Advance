    ...
    def generate_summary_pdf(config: Dict, tree: Dict, requirements: str) -> bytes:
        ...
        try:
            ...
            def section_header(title: str):
                ...
                pdf.cell(usable_width, 8, txt=_ascii(title), ln=True, fill=True)
                pdf.set_text_color(0, 0, 0)
                pdf.ln(2)
            ...
            for key, value in config.items():
                ...
                if str(key).lower() == 'description':
                    continue
                # Ensure we have proper margins before rendering
                pdf.set_x(10)
                text = _ascii(f"- {key}: {value}")
                # Truncate extremely long values to prevent rendering issues
                if len(text) > 500:
                    text = text[:497] + "..."
                # Calculate usable width (page width - left margin - right margin)
                usable_width = pdf.w - pdf.l_margin - pdf.r_margin
                pdf.multi_cell(usable_width, 6, txt=text)
            pdf.ln(2)
            ...
            if steps:
                ...
                for line in steps:
                    ...
                    pdf.set_x(10)
                    usable_width = pdf.w - pdf.l_margin - pdf.r_margin
                    pdf.multi_cell(usable_width, 6, txt=_ascii(line))
                pdf.ln(2)
            ...
            if desc:
                ...
                pdf.set_x(10)
                usable_width = pdf.w - pdf.l_margin - pdf.r_margin
                pdf.multi_cell(usable_width, 6, txt=_ascii(str(desc)))
                pdf.set_text_color(0, 0, 0)
                pdf.ln(2)
            ...
            # Output as bytes (fpdf returns bytearray when dest='S')
